...
（1.22.1-51-gcd2a6f61）を使用し、16 論理 CPU、61 GiB RAM、Ubuntu 26.04 LTS の
同じ環境で、いずれもシングルスレッドで実行した。C の基準実装は gcc 15.2.0（`-O2`）、`hts.cr`
...
スループットの中央値を示す。初回実行後は入力全体がページキャッシュに収まるため、三つの
実装は同じキャッシュ条件で比較した。データ生成と測定に用いたスクリプトは `benchmark`
ディレクトリに収録している。
